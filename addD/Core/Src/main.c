...
int main(void)
{
  ...
  while (1)
  {
    idle_counter++;  // CPU가 놀고 있는 시간 측정

    // Polling 방식으로 UART 수신 (timeout: 10ms)
    if (HAL_UART_Receive(&huart2, &receivedChar, 1, 10) == HAL_OK)
    {
      if (receivedChar != '\r' && receivedChar != '\n') // Enter 무시
      {
        switch (receivedChar)
        {
          case '1': outputChar = 'A'; break;
          case '2': outputChar = 'B'; break;
          case '3': outputChar = 'C'; break;
          default : outputChar = '?'; break;
        }
