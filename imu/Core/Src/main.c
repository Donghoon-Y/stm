...
int main(void)
{
  ...
  while (1)
  {
    ...
    float X = angleX-baseangleX ;
    float Y = angleY-baseangleY ;
    ...
    float roll = alpha * (roll + (gyroX - baseGyroX*dt)) + (1-alpha)*angleX ;
    float pitch = alpha * (pitch + (gyroY - baseGyroY*dt)) + (1-alpha)*angleY ;
    float yaw = gyroZ_1;
